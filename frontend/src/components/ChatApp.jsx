...
const ChatScreen = ({ currentUser, selectedUser, onBack, apiUrl }) => {
  ...
  const SOCKET_URL = apiUrl; // Use the same URL passed as apiUrl
  ...
  // Toggle sidebar function - IMPORTANT: This was missing and causing the error
  const toggleSidebar = () => {
  ...
  useEffect(() => {
    ...
    socketRef.current.on("connect_error", (error) => {
      ...
      if (reconnectAttempts.current++ > 3) {
        setError(`Connection issue: ${error.message}. Try refreshing the page.`);
      }
    });
    ...
    socketRef.current.on("private_message", (message) => {
      ...
      // Skip if no selected user (shouldn't happen, but just in case)
  ...
  }, [currentUser, SOCKET_URL, selectedUser, conversationId]);
